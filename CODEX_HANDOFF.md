...
- `AIOrchestrator` routes PixelLab base creation, text animation, and rotations; fal.ai remains the concept and compatible base fallback. The skeleton (`precise`) path is rejected at the API boundary until its placeholder pose interpolation is replaced.
